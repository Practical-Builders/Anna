...
app.post("/screenshot", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.warn(`[server] Navigation did not fully complete (${navErr.message}). Attempting screenshot anyway...`);
    }
    ...
    for (const selector of PRODUCT_IMAGE_SELECTORS) {
      ...
      try {
        ...
        console.log(`[server] Element screenshot captured (${Math.round(screenshotBuffer.length / 1024)} KB).`);
        break;
      } catch (elErr) {
        console.warn(`[server] Element screenshot failed for "${selector}": ${elErr.message}`);
      }
    }

    // --- Fallback: find the largest <img> by natural dimensions ---
    if (!screenshotBuffer) {
      console.log("[server] No selector matched. Searching for largest image by natural dimensions...");

      const largestImgSrc = await page.evaluate(() => {
        const imgs = Array.from(document.querySelectorAll("img"));
        const best = imgs.reduce((prev, img) => {
      ...
      if (largestImgSrc) {
        ...
        if (isVisible) {
          try {
            screenshotBuffer = await element.screenshot();
            console.log(`[server] Largest-image screenshot captured (${Math.round(screenshotBuffer.length / 1024)} KB).`);
          } catch (elErr) {
            console.warn(`[server] Largest-image screenshot failed: ${elErr.message}`);
          }
        }
      }
    }
    ...
    if (!screenshotBuffer) {
      console.warn("[server] Could not isolate a product image — falling back to full-page screenshot.");
      screenshotBuffer = await page.screenshot({ fullPage: true });
      console.log(`[server] Full-page screenshot captured (${Math.round(screenshotBuffer.length / 1024)} KB).`);
    }
